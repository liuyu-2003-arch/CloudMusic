...
const MusicGrid: React.FC<MusicGridProps> = ({ title, songs, onPlay }) => {
  return (
    <div className="mb-10">
      <div className="flex items-center justify-between mb-4 px-1">
        <h2 className="text-2xl font-bold text-apple-text">{title}</h2>
        <button className="text-apple-accent text-sm font-medium hover:underline">See All</button>
      </div>
      
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-x-6 gap-y-8">
        {songs.map((song) => (
          <div key={song.id} className="group relative flex flex-col cursor-pointer" onClick={() => onPlay(song)}>
            {/* Cover Image Container */}
            <div className="relative aspect-square w-full overflow-hidden rounded-xl shadow-md bg-gray-200 mb-3 transition-transform duration-300 group-hover:scale-[1.02]">
              <img 
                src={song.coverUrl} 
                alt={song.title} 
